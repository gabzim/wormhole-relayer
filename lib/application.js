...
class RelayerApp {
    ...
    async fetchVaa(chain, emitterAddress, sequence, retryAttempts = 3, retryTimeout = 200) {
        const vaa = await (0, wormhole_sdk_1.getSignedVAAWithRetry)(this.opts.wormholeRpcs, Number(chain), emitterAddress.toString("hex"), sequence.toString(), { transport: grpcWebNodeHttpTransport.NodeHttpTransport() }, retryTimeout, retryAttempts);
    ...
    storageKoaUI(path) {
        ...
        (0, api_1.createBullBoard)({
            queues: [
                new bullMQAdapter_1.BullMQAdapter(this.storage.vaaQueue),
                new bullMQAdapter_1.BullMQAdapter(this.storage.batchVaaQueue),
            ],
        ...
        let chainRouting = async (ctx, next) => {
            if (!ctx.vaa) {
                ...
                for (const vaa of ctx.batchVaa.vaas) {
                    let router = this.chainRouters[vaa.emitterChain];
                    if (!router) {
                        continue;
                    }
